      ...
      <div className="space-y-3">
        {data.episodes.length ? (
          data.episodes.map((ep) => (
            <div
              key={ep.id}
              className="flex gap-4 rounded border border-zinc-800 bg-zinc-900/60 p-3 transition-all hover:border-red-600/40"
            >
              {ep.still_path ? (
                <img src={getImageUrl(ep.still_path)} alt={ep.name} className="h-20 w-32 shrink-0 rounded object-cover" />
              ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  {ep.vote_average > 0 && (
                    <span className="ml-auto flex items-center gap-1 text-xs font-bold text-yellow-400">
                      <FiStar size={12} /> {ep.vote_average.toFixed(1)}
                    </span>
                  )}
                </div>
